...
import { HandoverItemModal } from '@/components/common/inventory/modals/inventory-handover-modal'
import { ArchiveConfirmModal } from '@/components/common/inventory/modals/inventory-archive-modal'
...
export function InventoryDetailPage() {
  const { slug, itemId } = useParams({ from: '/console/$slug/staff/inventory/$itemId/' })
          ...
          item && item.status !== 'Returned' && item.status !== 'Archived' && item.status !== 'Expired' ? (
            <>
              ...
              <Button
                ...
                type="button"
                className="border-[#222222] text-[#222222] hover:bg-[#222222] hover:text-white transition-all hover:scale-[1.03] hover:drop-shadow-sm"
                onClick={() => navigate({ to: `/console/${slug}/staff/inventory/${item.id}/edit` })}
              >
                Edit
              </Button>
              <Button
          ...
          item ? (
            <>
              <HandoverItemModal
                ...
                onSuccess={() => {
                  void navigate({ to: `/console/${slug}/staff/inventory/${item.id}/`, replace: true } as any)
                }}
              />
              <ArchiveConfirmModal
                open={archiveOpen}
