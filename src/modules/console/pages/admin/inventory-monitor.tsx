...
export function AdminInventoryMonitorPage() {
  ...
  // Admin only filters
  const [intakeStaffId, setIntakeStaffId] = useState('')
  ...
  const debouncedIntakeStaff = useDebouncedValue(intakeStaffId.trim(), 500)
  const debouncedReturnStaff = useDebouncedValue(returnStaffId.trim(), 500)
  ...
  const listParams = useMemo(
    () => ({
      ...
      intakeStaffId: debouncedIntakeStaff || undefined,
      returnStaffId: debouncedReturnStaff || undefined,
    }),
    [currentPage, debouncedSearch, statusFilter, categoryFilter, debouncedColor, debouncedBrand, fromDate, toDate, debouncedIntakeStaff, debouncedReturnStaff],
  )
  ...
  useEffect(() => {
    setCurrentPage(1)
  }, [debouncedSearch, statusFilter, categoryFilter, debouncedColor, debouncedBrand, fromDate, toDate, debouncedIntakeStaff, debouncedReturnStaff])
  ...
  return (
    <Layout>
      <div className="p-4 sm:p-4 lg:p-6 min-h-screen sm:mx-4">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-4">
          <div>
            ...
            <p className="text-gray-600 text-xs sm:text-base">
              View organization's lost and found items. Search by name, details, or staff IDs.
            </p>
          </div>
        ...
        {/* Filters Section (inline wrapping layout with admin controls) */}
        <div className="flex flex-wrap items-center gap-4 bg-white p-3 rounded-lg border border-gray-200 shadow-sm mb-6 text-sm">
          ...
          <div className="flex items-center gap-2">
            <span className="text-gray-500 font-medium whitespace-nowrap">Intake Staff</span>
            <select
              value={intakeStaffId}
              onChange={(e) => setIntakeStaffId(e.target.value)}
              className="w-32 pl-2 pr-6 py-1.5 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 text-gray-900 bg-white text-ellipsis"
            >
          ...
          <div className="flex items-center gap-2">
            <span className="text-gray-500 font-medium whitespace-nowrap">Return Staff</span>
            <select
              value={returnStaffId}
              onChange={(e) => setReturnStaffId(e.target.value)}
              className="w-32 pl-2 pr-6 py-1.5 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 text-gray-900 bg-white text-ellipsis"
            >
              <option value="">All</option>
              {members.map(m => (
          ...
          {(searchTerm || statusFilter !== ALL_STATUS || categoryFilter !== ALL_STATUS || colorFilter || brandFilter || fromDate || toDate || intakeStaffId || returnStaffId) && (
            <button
              onClick={() => {
                setSearchTerm('')
              ...
              >
                ...
                <div className="p-4 flex flex-col flex-1">
                  ...
                  <p className="text-sm text-gray-600 mb-3 line-clamp-2 leading-5 min-h-[40px]">
                    {item.item.additionalDetails?.trim() || ''}
                  </p>
                  ...
                  <Link to="/console/$slug/admin/inventory/$itemId" params={{ slug, itemId: item.id }} className="mt-auto block">
