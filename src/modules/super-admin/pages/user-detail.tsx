  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-xl font-bold text-gray-900">{sub.planSnapshot.name}</p>
      </div>
      ...
      <div className="pt-2 border-t border-gray-200">
        <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-gray-600">
          <div>
            Billing: <span className="font-medium text-gray-900">{sub.planSnapshot.billingInterval}</span>
          </div>
          <div>
            Price:{' '}
          ...
          <div>
            QR codes (user): <span className="font-medium text-gray-900">{qrTotal}</span>
          </div>
        </div>
      </div>
      <p className="text-sm text-gray-600 pt-2 border-t border-gray-200">{planDescription(sub)}</p>
    </div>
  );
}
      ...
      <Layout>
        <div className="p-8 bg-gray-50 min-h-screen">
          <div className="bg-white rounded-xl shadow-sm p-8 text-center max-w-lg mx-auto">
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Could not load user</h2>
      ...
      <div className="p-8 bg-gray-50 min-h-screen">
        <div className="mb-6">
          <nav className="text-sm text-gray-500">
            <button type="button" onClick={handleBack} className="hover:text-gray-700 cursor-pointer">
              User Management
            </button>
        ...
        <div className="bg-white rounded-xl shadow-sm p-8 mb-6">
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Global role</label>
                <div className="w-full px-4 py-2 border border-gray-200 rounded-lg bg-gray-50 text-gray-900">
            ...
            <div className="space-y-4">
              {sub ? (
                ...
                <>
                  ...
                  <p className="text-gray-900 font-medium">No active subscription</p>
                  <p className="text-sm text-gray-600 pt-2">
                    Total QR codes (user):{' '}
                    <span className="font-medium text-gray-900">{data.qrUsage.totalQrCodes}</span>
                  </p>
                </>
              )}
            </div>
                  ...
                  <tbody>
                    {data.billingHistory.slice(0, 6).map((invoice) => (
                      <tr key={invoice.id} className="border-t">
                        <td className="py-3 pr-4 text-gray-700">{invoice.providerInvoiceId}</td>
                        <td className="py-3 pr-4 text-gray-700">{formatDate(invoice.paymentDate)}</td>
                        <td className="py-3 pr-4 text-gray-700">
                          {invoice.amount.toFixed(2)} {invoice.currency}
                        </td>
                        <td className="py-3 text-gray-700">{paymentStatusLabel(invoice.status)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
