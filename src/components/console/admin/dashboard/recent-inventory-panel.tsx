...
function InventoryRow({ item }: { item: AdminInventoryItem }) {
  return (
    <div className="flex items-center gap-3 py-2.5 px-3 rounded-xl hover:bg-[#f7f7f7] transition-colors">
      <div className="w-9 h-9 rounded-lg bg-[#f7f7f7] flex items-center justify-center flex-shrink-0">
...
interface RecentInventoryPanelProps {
...
export function RecentInventoryPanel({ items, totalCount, page, pageSize, onPageChange }: RecentInventoryPanelProps) {
  ...
  const end = totalCount > 0 ? Math.min(page * pageSize, totalCount) : page * pageSize
      ...
      {items.length > 0 && (
        <div className="flex items-center justify-between pt-3 mt-2 border-t border-[#f7f7f7]">
          <span className="text-[11px] text-[#929292]">
            {start}–{end}{totalCount > 0 ? ` of ${totalCount}` : ''}
          </span>
