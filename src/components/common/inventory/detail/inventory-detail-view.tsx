...
import { useChatConversationsByPostId, useChatMessages } from '@/hooks/use-chat'
...
import type { IConversation, IConversationPartner } from '@/types/chat.types'
...
function ConnectedClaimConversation({
                  ...
                  <div className="space-y-3">
                    {relatedConvs.map((conv) => {
                      ...
                      const isAssignedToOtherStaff =
                        !!conv.assignedStaff?.id && !!myUserId && conv.assignedStaff.id !== myUserId
                      const isLocked = !!conv.assignedStaff?.id && (!myUserId || isAssignedToOtherStaff)
                      return (
                        <button
                          key={conv.id}
                          type="button"
                        ...
                        >
                          <div className="flex items-start justify-between gap-3">
                            ...
                            <div className="shrink-0 flex flex-col items-end gap-1">
                              <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold ${STATUS_BADGE[conv.status]}`}>
