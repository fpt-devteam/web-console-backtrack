...
interface ClaimVerifyDialogProps {
  partnerName: string
...
export function ClaimVerifyDialog({ partnerName, avatarUrl, isPending, onConfirm, onCancel }: ClaimVerifyDialogProps) {
  return (
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-[#222222] text-sm">Mark claim as verified?</h3>
              <p className="text-xs text-[#929292] mt-0.5">Confirm the claimant owns this item</p>
            </div>
          </div>
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={onConfirm}
            disabled={isPending}
            className="cursor-pointer flex-1 px-4 py-2.5 text-sm font-medium text-white bg-blue-600 rounded-xl hover:bg-blue-700 transition-colors disabled:opacity-50 flex items-center justify-center gap-1.5"
          >
            {isPending ? (
